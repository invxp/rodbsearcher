...
func Daemonize(isDaemonize bool, argFlag, daemonizeFlag string) {
	if len(os.Args) > 2 && os.Args[1] == "-"+daemonizeFlag {
		log.Printf("rodbsearcher %d run in daemonize: %v, ppid: %d\n", os.Getpid(), os.Args, os.Getppid())
		return
	}

	if !isDaemonize {
		log.Printf("rodbsearcher %d run in frontend: %v, ppid: %d\n", os.Getpid(), os.Args, os.Getppid())
		return
	}
	...
	log.Printf("rodbsearcher %d prepare to run in daemonize: %s\n", os.Getpid(), runArg)
	...
	log.Printf("rodbsearcher %d exit for daemonize: %s, release: %v\n", os.Getpid(), runArg, cmd.Process.Release())
